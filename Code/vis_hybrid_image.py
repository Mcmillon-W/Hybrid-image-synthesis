...
def  vis_hybrid_image(hybrid_image):
  ...
  output = hybrid_image/255
  cur_image = hybrid_image/255
  for i in range(2,scales+1):
      # add padding
      output = np.concatenate((output,np.ones((original_height, padding, num_colors))),axis=1) #implement the correct command here
      # dowsample image
