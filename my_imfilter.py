...
def my_imfilter(image,filter):  #which will work identically to the function below
    (img_len,img_wid,channel)=image.shape
    (fil_len,fil_wid)=filter.shape
    copy=np.zeros_like(image)
    for i in range(channel):
        for x in range(img_len):
            for y in range(img_wid):
                response=0
                for u in range(-(fil_len//2),fil_len//2+1):
                    for v in range(-(fil_wid//2),fil_wid//2+1):
                        if(x+u<0 or x+u>=img_len):
                          continue
                        elif(y+v<0 or y+v>=img_wid):
                          continue
                        response+=filter[u+fil_len//2][v+fil_wid//2]*image[x+u][y+v][i]
                copy[x][y][i]=response
    return copy       
